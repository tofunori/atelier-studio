...
The 1.3.6 release candidate is gated by:

- TypeScript type checking and a production Vite build.
- The complete frontend, sidecar, gallery, protocol, Rust application, and Rust workspace test suites.
- Gallery route parity, the gallery diff suite, and Playwright end-to-end coverage.
- A canonical local Apple Silicon DMG build from `main`.
- A live packaged-app relaunch from the canonical checkout with Rust sidecar health verification.
- The tag-triggered GitHub Actions release workflow, which repeats verification, builds the public DMG, verifies the embedded Node.js runtime, and publishes the release only after all gates pass.

## Complete commit ledger

The following 64 product and engineering commits are included after `v1.3.5` and before the 1.3.6 release metadata commit. Descriptions are normalized into English; hashes link the ledger to the immutable Git history.
