...
export default function AtelierPane({
  ...
  // Pont vers le rail (plan 057) : la bande d'onglets du pane a disparu, c'est
  // la colonne de gauche qui les porte. Le workspace reste la source de vérité
  // — il publie ses onglets, et reçoit sélection et fermeture par événements
  // plutôt que par des refs remontées à travers trois composants.
  useEffect(() => {
    const pane = findWorkspacePane(workspace.root, workspace.focusedPaneId) ?? listWorkspacePanes(workspace.root)[0];
  ...
  useEffect(() => {
    const onSelect = (event: Event) => {
      ...
      const pane = findWorkspacePane(workspace.root, workspace.focusedPaneId) ?? listWorkspacePanes(workspace.root)[0];
      const ref = pane?.tabs.find((candidate) => workspaceTabId(candidate) === id);
      ...
      <div className={`workspace-pane-controls is-${placement}`} data-pane-controls={placement}>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" sideOffset={5}>
            ...
            {/* plan 057 : actions rescapées du menu contextuel des onglets —
                sans elles, épingler, colorer et inspecter disparaissaient avec
                la bande. */}
