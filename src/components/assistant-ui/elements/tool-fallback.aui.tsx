...
import { Textarea } from "@/components/assistant-ui/primitives/textarea";

const ANIMATION_DURATION = 200;

const pressable = "tw:active:scale-[0.98]";
...
function ToolFallbackRoot({
  ...
  return (
    <Collapsible
      ...
      className={cn(
        "aui-tool-fallback-root tw:group/tool-fallback-root tw:w-full",
        className,
      )}
      style={
        {
...
function ToolFallbackDuration({
  ...
  return (
    <span
      ...
      className={cn(
        "aui-tool-fallback-duration tw:text-muted-foreground tw:text-xs tw:tabular-nums",
        className,
      )}
      {...props}
    >
...
function ToolFallbackTrigger({
  ...
  return (
    <CollapsibleTrigger
      ...
      className={cn(
        "aui-tool-fallback-trigger tw:group/trigger tw:text-muted-foreground tw:hover:text-foreground tw:flex tw:w-fit tw:origin-left tw:items-center tw:gap-2 tw:py-1.5 tw:text-sm tw:transition-[color,scale] tw:active:scale-[0.98]",
        className,
      )}
      {...props}
    >
      <Icon
        ...
        className={cn(
          "aui-tool-fallback-trigger-icon tw:size-4 tw:shrink-0",
          isCancelled && "tw:text-muted-foreground",
          isRunning && "tw:animate-spin tw:[animation-duration:0.6s]",
        )}
      />
      <span
        data-slot="tool-fallback-trigger-label"
        className={cn(
          "aui-tool-fallback-trigger-label-wrapper tw:inline-block tw:text-start tw:leading-none",
          isCancelled && "tw:text-muted-foreground tw:line-through",
          isRunning && "tw:shimmer tw:motion-reduce:animate-none",
        )}
      >
        {label}: <b>{toolName}</b>
      </span>
      ...
      <ChevronDownIcon
        ...
        className={cn(
          "aui-tool-fallback-trigger-chevron tw:size-4 tw:shrink-0",
...
function ToolFallbackContent({
  ...
  return (
    <CollapsibleContent
      ...
      className={cn(
        "aui-tool-fallback-content tw:relative tw:overflow-hidden tw:text-sm tw:outline-none",
    ...
    >
      <div
        className={cn(
          "tw:flex tw:flex-col tw:gap-2 tw:ps-6 tw:pt-1 tw:pb-2 tw:ease-[cubic-bezier(0.32,0.72,0,1)] tw:motion-reduce:animate-none",
          "tw:group-data-open/collapsible-content:animate-in tw:group-data-open/collapsible-content:fade-in-0 tw:group-data-open/collapsible-content:blur-in-[2px] tw:group-data-open/collapsible-content:slide-in-from-top-1",
          "tw:group-data-closed/collapsible-content:animate-out tw:group-data-closed/collapsible-content:fade-out-0 tw:group-data-closed/collapsible-content:blur-out-[2px] tw:group-data-closed/collapsible-content:slide-out-to-top-1",
          "tw:group-data-closed/collapsible-content:animation-duration-(--animation-duration) tw:group-data-open/collapsible-content:animation-duration-(--animation-duration)",
        )}
      >
        {children}
      </div>
...
function ToolFallbackArgs({
  ...
  return (
    <div
      ...
      {...props}
    >
      <pre className="aui-tool-fallback-args-value tw:bg-muted/50 tw:text-foreground/90 tw:rounded-md tw:p-2.5 tw:text-xs tw:whitespace-pre-wrap">
        {argsText}
      </pre>
    </div>
  );
...
function ToolFallbackResult({
  ...
  return (
    <div
      ...
      {...props}
    >
      <p className="aui-tool-fallback-result-header tw:text-muted-foreground tw:text-xs tw:font-medium">
        Result:
      </p>
      <pre className="aui-tool-fallback-result-content tw:bg-muted/50 tw:text-foreground/90 tw:mt-1 tw:rounded-md tw:p-2.5 tw:text-xs tw:whitespace-pre-wrap">
...
function ToolFallbackError({
  ...
  return (
    <div
      ...
      {...props}
    >
      <p className="aui-tool-fallback-error-header tw:text-muted-foreground tw:font-semibold">
        {headerText}
      </p>
      <p className="aui-tool-fallback-error-reason tw:text-muted-foreground">
        {errorText}
      </p>
    </div>
  );
...
function ToolFallbackApproval({
  ...
  const promptText = approval?.prompt ? (
    <p className="aui-tool-fallback-approval-prompt tw:text-foreground">
      {approval.prompt}
    </p>
  ) : null;
  ...
  const errorText = error ? (
    <p
      role="alert"
      className="aui-tool-fallback-approval-error tw:text-destructive tw:text-xs"
    >
      {error}
    </p>
  ) : null;
  ...
  const answerField = acceptsText ? (
    <div className="aui-tool-fallback-approval-answer tw:flex tw:flex-col tw:items-start tw:gap-2">
      <Textarea
        value={answer}
  ...
  if (confirming) {
    ...
    return (
      <div
        ...
        className={cn(
          "aui-tool-fallback-approval-confirm tw:flex tw:flex-col tw:gap-2 tw:pt-1",
          className,
        )}
        {...props}
      >
        <p className="aui-tool-fallback-approval-confirm-title tw:font-semibold">
          {confirmMeta?.title ?? `${approvalOptionLabel(confirming)}?`}
        </p>
        {confirmDescription && (
          <p className="aui-tool-fallback-approval-confirm-description tw:text-muted-foreground">
            {confirmDescription}
          </p>
        )}
        {confirming.grants && confirming.grants.length > 0 && (
          <ul className="aui-tool-fallback-approval-confirm-grants tw:flex tw:flex-col tw:gap-1">
            {confirming.grants.map((grant) => (
              <li key={grant}>
                <code className="aui-tool-fallback-approval-confirm-grant tw:bg-muted tw:rounded tw:px-1.5 tw:py-0.5 tw:text-xs">
                  {grant}
                </code>
              </li>
            ))}
  ...
  if (declaredOptions && declaredOptions.length > 0) {
    ...
    return (
      <div
        ...
        className={cn(
          "aui-tool-fallback-approval tw:flex tw:flex-col tw:gap-2 tw:pt-1",
          className,
        )}
        {...props}
      >
  ...
  return (
    <ToolFallbackRoot open={open} onOpenChange={setOpen}>
      ...
      <ToolFallbackContent>
        ...
        <ToolFallbackArgs
          argsText={argsText}
          className={cn(isCancelled && "tw:opacity-60")}
        />
