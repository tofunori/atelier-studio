...
export function GitCommitsView({ controller }: { controller: GitSurfaceController }) {
  ...
  return (
    <div className="git-commits-view">
      ...
      <AlertDialog open={resetOpen} onOpenChange={setResetOpen}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{t("git.reset-confirm")}</AlertDialogTitle><AlertDialogDescription>{t("git.reset-help")}</AlertDialogDescription></AlertDialogHeader><SegmentedControl label={t("git.reset-confirm")} value={resetMode} onChange={setResetMode} options={[{ value: "soft", label: "Soft" }, { value: "mixed", label: "Mixed" }, { value: "hard", label: "Hard" }]} /><AlertDialogFooter><AlertDialogCancel>{t("action.cancel")}</AlertDialogCancel><AlertDialogAction variant={resetMode === "hard" ? "destructive" : "default"} onClick={() => controller.historyAction("gitResetToCommit", { sha: details?.sha, mode: resetMode })}>{t("git.reset-advanced")}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
