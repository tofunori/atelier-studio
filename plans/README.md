...
| 070 | Reprise des messages d'agents liés coincés en « delivering » après un crash | P2 | S | — | REVISE → 070b (exécuteur sonnet, 2026-09-06) : diff correct et testé (branche `advisor/070-mailbox-delivering` [28cc36f4] : `stuck_delivering()`, réconciliation par le journal dans `drain_mailbox`, +3 tests, 189+22 verts) MAIS il a découvert que `server.rs:~209` appelle déjà `recover_delivering` au boot — une remise en file AVEUGLE qui vide `stuck_delivering()` avant la réconciliation. Le défaut de production est donc l'inverse du constat initial : risque de **double livraison** au redémarrage. 070b retire l'appel aveugle (hors scope de 070) |
| 070b | Boot : la reprise des « delivering » passe par la réconciliation du journal, plus par la remise en file aveugle | P2 | S | 070 | DONE (exécuteur sonnet, revue 2026-09-06 : APPROVE — branche `advisor/070b-boot-reprise-journal` [2e54f1e2], **contient 070 et main** ; test rouge reproduisant la double livraison (1 vs 4 événements) puis vert ; `recover_delivering` supprimé partout ; 195 + 21 tests verts. C'est CETTE branche à fusionner pour clore le défaut (070 seule ne suffit pas)) |
| 071 | Caractérisation : parité i18n FR/EN et câblage de `Chat.tsx` | P2 | M | — | DONE (v2, 2026-09-06) — branche `advisor/071-caracterisation-i18n-chat` (58915cc0, 01ca2abd), revue APPROVE : 14/14 verts, `Chat.tsx` intact, périmètre = `i18n.ts` (export fr/en) + 2 fichiers de test ; cas 5/6 assertent les arguments ; règle de parité tolère `*Plural`. Historique v1 : STOP justifié (plan réécrit depuis le code). Base : main mergé |
| 072 | Caractérisation `ws_router.rs` : un test par famille de messages | P2 | L | — | DONE (2026-09-06) — branche `advisor/072-caracterisation-ws-router` (ce9f6bcc, base 5b7eac55), revue APPROVE : 224 tests runtime ok, 34 → 66 `#[tokio::test]`, checklist 121 types (67 cochés), diff limité aux modules de test. **Bug trouvé** : `deleteAutomation` avec un id existant ne répond jamais (interblocage `automations.rs:63`, garde du Mutex tenue pendant le `match` puis `automations_reply` re-verrouille) → plan 078. `zoteroCollections` lit la vraie base `~/Zotero` (pas d'injection de chemin) — test écrit sur la forme commune. Non couverts : `generateImage` (réseau), `git*` (20), `term*` (4), articleImport*, apiProviders, sessions/export, agents liés, quickAsk, etc. (54 types, listés dans la checklist) |
...
| 074 | Passerelle distante fail-closed (pair inconnu, liste d'hôtes vide) | P1 | S | — | DONE (exécuteur sonnet, revue 2026-09-06 : APPROVE — branche `advisor/074-passerelle-fail-closed` [4f17d795, 2982c1c1] **basée sur 52c4d4b0, à rebaser** ; `peer == "unknown"` refusé, liste vide → 403 `host_not_configured`, avertissement au démarrage ; 6 tests (unitaires + intégration) assertant les codes ; suites remote vertes ; prod : `remote_gateway.rs:298` construit toujours une liste non vide (vérifié). Déviation acceptée : `check_host` appelé dans `require_admin` — les routes admin ignoraient le Host) |
...
| 076 | i18n par locale : l'entrée ne charge qu'une langue, budget retrouvé | P2 | M | 071 | DONE (2026-09-06) — branche `advisor/076-i18n-par-locale` (1bb6cc2a, d69382dc, a451ae44 ; base = main + 071), revue APPROVE : entrée 1023 → **872 Ko**, chunks `fr-*`/`en-*` (81/75 Ko), garde-fou `i18n/en` prouvé en négatif, aucun import direct des dictionnaires, `npm run verify` ✓ (2 timeouts `App.orchestration` = flake, 49/49 isolé). Déviations acceptées : `src/test/setup.ts` précharge fr+en (50 tests appellent `setLanguage` en synchrone) ; `setLanguage` bascule synchrone si le dictionnaire est en registre, sinon après le chunk avec garde `languageSetting === next`. **À faire sur main** : committer les non-suivis `src/components/biblio/*`, `src/lib/usageSummary.ts`, `gallery/assets/cm6/latex_lang/`, `rust/crates/atelier-gallery/src/ranged.rs`, `rust/crates/atelier-remote/src/zotero_routes.rs` (sinon tsc/build/cargo cassent sur un clone propre) |
| 077 | État galerie : fusion par présence + version, refus des snapshots périmés (409) | P2 | M | — | DONE (exécuteur sonnet, revue 2026-09-06 : APPROVE — branche `advisor/077-etat-galerie-version` [93dbb2c9, a3a61204], **base = main fusionné (a95d99c8)**, 3 fichiers ; `rev` lu/écrit/incrémenté dans `/state` et `toggle_favorite`, 409 `state_stale` avec l'état courant, fusion par présence, client : `stateRev`, une seule relance sur 409 puis avis discret (jetons seulement) ; 5 tests Rust + 4 tests template, parity + diff suite ok, syntaxe ok) |
| 078 | Mode lecture PDF (reflow une colonne, typographie, annotations) | P2 | L | lot 1 fluidité (2b453afd, 2f610491) | DONE (branche advisor/078-mode-lecture, commits e2fc1032..8d663356 + résiduels ; 7 tâches, 3 tours de correction puis une vague de correction finale après revue de branche : ordre de lecture de la manchette, titres en gras, fragments orphelins, dé-césure du texte affiché, page des rects, délai de 60 s sur pdftohtml, marques ul/st, navigation, détails ; résiduels : garde de manchette (col1_top > 15 % de la hauteur de page), seuil d'exposant 0,8, test pinnant le texte complet d'une sélection à cheval, retrait du reformatage `cargo fmt` hors plan ; vérifié sur un vrai article Copernicus de 19 pages — 1 → 19 titres, 104 → 21 blocs math ; à fusionner sur main après relance) |
| 078 | `deleteAutomation` : interblocage du Mutex (garde tenue pendant le `match`) | P1 | S | 072 | TODO — trouvé par 072 |
...
- « Message d'agent lié livré deux fois après crash » : **rejeté à tort le 2026-09-06** — le réviseur n'avait pas vu `server.rs::serve_once` qui appelle `recover_delivering` (remise en file aveugle) au boot. Le risque de double livraison existe bien sur ce chemin ; corrigé par 070 + 070b.
