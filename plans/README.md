...
| 024 | Transformer le panneau Projets en Research Navigator du projet actif | P1 | M | 014–016 | IN PROGRESS — implémenté (Fable 5, 2026-07-10, branche `claude/atelier-projects-panel-redesign-f86165`) : modèle pur deriveProjectNavigatorModel [19 tests, 12 règles], panneau = projet actif seul (header identité + Nouveau chat primaire + recherche locale + overflow Resume/Finder/personnaliser/retirer) ou Chats sans projet, sections Continuer/Épinglés/Conversations sans doublon, lignes accessibles (bouton natif, aria-current, slot terminal 48 px heure⇄actions sans layout shift [vérifié Playwright : avant=false/après=true], menu ⋯ clavier via primitives ui), CSS Precision Native (tokens seuls, 14 px supprimés, reduced-motion arc corrigé, container query ≤220 px), banc #navbench + 17 captures avant/après docs/ui/projects-panel/ ; 272 tests frontend verts — DONE attend la revue Codex |
...
| 020 | Recomposer les tours de chat et le composer | P1 | L | 015–016, 018, 025 | IN PROGRESS — implémenté (Fable 5, 2026-07-10, branche `claude/atelier-020-chat-composer` sur la base 024) : matrice événements→présentation consommant le contrat 025 (docs/ui/020-event-presentation-matrix.md), anatomie du tour Demande→Activité (« Activité · N étapes · durée », erreurs hors pli)→Réponse→ResultCapsule honnête (statut/usage/« Usage indisponible »/fichiers via diff à la demande/Annuler le tour/review — jamais de section tests sans événement), composer hiérarchisé (effort en popover modèle avec slider clavier role=slider, résumé « · effort » dans le bouton, menus +/modèle en vrais boutons role=menuitem* flèches+Échap, Send/Stop unique action primaire), actions de tour visibles au focus, chevrons texte remplacés (Tick SVG), centrage colonne 760 px réparé (margin-shorthand préexistant), banc #chatbench + 20 captures avant/après docs/ui/chat/ ; contrats préservés (Enter/Shift+Enter/IME, steer/queue/stop, catalogue sidecar, permissions, attachments) ; 289 tests frontend verts — DONE attend la revue Codex |
| 021 | Finaliser Settings, accessibilité, responsive et QA visuelle | P1 | L | 017–020 | IN PROGRESS — implémenté (Fable 5, 2026-07-10, branche `claude/atelier-021-settings-a11y` sur la base 020) : A) Settings — nav aria-current 2 signaux, Échap, select compact ≤880 px, confirmations destructives (restore/provider/images), SegmentedControl thème+densité, theme-rows boutons, InlineNotice + Copier le diagnostic (décision : pas de retrofit SurfaceHeader, wireframe D = référence conservée) ; B) audit a11y complet + top 15 corrigé — bug de closure Échap global (interrupt intempestif), SurfaceHeader h2, Research Home h1, palette combobox aria-activedescendant, composer (étoile favori bouton frère, anneau contexte focusable), reviewer-bar restructurée, ProjectStyleMenu factorisé Rail+Sidebar, focus-visible/reveals/reduced-motion transverses, checklist docs/ui/021-a11y-checklist.md (reste documenté : Explorer clavier, menus AtelierPane/Git — plans dédiés) ; C) audit responsive 4 largeurs + zoom : zéro scroll horizontal (docs/ui/021-responsive-audit.md) ; D) Banner role=status, notices Settings ; E) harnais golden : 13 baselines Chromium committées (bancs + #setbench + app déconnectée, horloge figée, fr-FR), test:visual en CI macos, jamais d'auto-update ; 298 tests frontend + 13 goldens verts — DONE attend la revue Codex |
| 023 | Appliquer une direction artistique et une finition cosmétique haut de gamme | P1 | M | 014, 021 | IN PROGRESS — implémenté (Fable 5, 2026-07-10, branche `claude/atelier-023-polish` sur la base 021) : palette graphite OKLCH calibrée dark+light (candidats bruts du plan rejetés — quasi noirs ; teinte 255 unifiée, paliers réguliers, contrastes vérifiés par script, accent light #cf630d), preset atelier synchronisé, composer élevé (--elev-soft, focus ring couleur), rail actif = fond + trace orange (bordure+liseré supprimés), command center sans contour + keycap ⌘K, typo balance/pretty, Send accent 6 px press .98, budget de mouvement appliqué (Stop/unread/busy/Working statiques — spinner unique boucle), hovers brightness → paliers, swaps display:none → cross-fade, transitions re-tokenisées ≤150 ms, codeblock concentrique, activité inset ; tests de goût 8/8 (docs/ui/polish/README.md, avant/après committés) ; + série de retouches de goût validées EN DIRECT avec Thierry (aperçu navigateur) : zéro bouton orange app-wide (primaires neutres forts), identité projet UNE fois (crumb TopBar — panneau/Research Home/eyebrow chat dédupliqués), aucun badge/méta d'état dans l'en-tête du chat, capsule de fin de tour en ligne discrète (actions au hover/focus), pastille goal fermable optimistiquement, side-top et footer du panneau supprimés (usage → rail, au-dessus d'un VRAI engrenage Réglages), flyout du rail supprimé (cherry-picks 65c740e/dd91e58/987c0b5 de la branche principale, ProjectStyleMenu re-greffé) ; **galerie exclue** (directive session gallery/assets intouchable — CM6 Codex en cours ; passe galerie du plan à reporter) ; 298 tests + 13 goldens (baselines mises à jour, revue humaine demandée) — DONE attend la revue Codex |
| 022 | Réduire le coût de chargement du frontend final | P2 | M | 023 | DONE (2026-07-10, branche claude/atelier-022-perf — entrée 1639→864 KB min / 477→267 KB gzip ; en attente revue Codex) |
