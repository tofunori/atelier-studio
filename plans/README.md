...
| 071 | Caractérisation : parité i18n FR/EN et câblage de `Chat.tsx` | P2 | M | — | BLOCKED puis raffiné (2026-09-06) : l'exécuteur a STOP à raison — (1) le test de parité a trouvé `git.staging-summary` avec `{stagedPlural}`/`{untrackedPlural}` en FR seulement (accord français légitime, règle du test trop stricte) ; (2) les cas 3 et 5 du plan décrivaient un glisser-déposer inexistant et un Quick Ask via `wsSend` alors qu'il passe par `CustomEvent("quick-ask-open")` — erreur du réviseur, plan réécrit en v2 (voir le fichier), re-dispatch à suivre |
