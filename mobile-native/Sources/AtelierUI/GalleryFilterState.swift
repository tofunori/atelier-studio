...
struct GalleryFilterState {
    static let types = ["Tous", "LaTeX", "PDF", "Figures", "Texte", "Python"]
    var query = ""
    var type = "Tous"
    var expanded = false
    ...
    func matches(_ item: GalleryArtifact) -> Bool {
        ...
        return (!favoritesOnly || item.favorite == true) && kindMatches && (query.isEmpty || item.name.localizedStandardContains(query))
    }
}
