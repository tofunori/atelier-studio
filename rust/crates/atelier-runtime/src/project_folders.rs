...
pub fn scope_changed(root: &str, before: &Value, after: &Value) -> bool {
    fn scope(root: &str, settings: &Value) -> Value {
        if let Some(config) = settings.get("projectFolders").and_then(|v| v.get(root)) {
            let mut entries: Vec<String> = config["folders"]
                .as_array()
                .into_iter()
                .flatten()
                .map(|f| json!([f["path"], f["access"]]).to_string())
                .collect();
            entries.sort();
            json!(entries)
        } else {
            json!({"legacy":settings["additionalDirectories"]})
        }
    }
...
pub fn writable(root: &str, settings: &Value, legacy: Option<&Value>) -> Vec<String> {
    ...
    legacy
        ...
        .filter(|p| PathBuf::from(p).is_absolute() && PathBuf::from(p).is_dir())
        .map(str::to_string)
        .collect()
}

pub fn context(root: &str, settings: &Value) -> String {
    let Some(config) = settings.get("projectFolders").and_then(|v| v.get(root)) else {
        return String::new();
    };
    ...
    while let Some(dir) = pending.pop() {
        ...
        for entry in entries {
            ...
            if name.starts_with('.')
                || matches!(
                    name.as_ref(),
                    "node_modules" | "target" | "__pycache__" | "dist" | "venv"
                )
            {
                continue;
            }
...
mod tests {
    use super::*;
    #[test]
    fn isolates_projects_and_deduplicates() {
        let a = tempfile::tempdir().unwrap();
        let b = tempfile::tempdir().unwrap();
        let root = a.path().to_str().unwrap();
        let other = b.path().to_str().unwrap();
    ...
    #[test]
    fn finds_nested_files_in_non_git_sources_without_following_links() {
        ...
        assert_eq!(files, vec!["figures/plot.pdf"]);
        assert!(!partial);
    }
    #[test]
    fn scope_change_ignores_gallery_and_names_but_detects_revocation() {
