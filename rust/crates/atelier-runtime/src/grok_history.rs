...
pub(crate) fn prefer_richer_dialogue(journal: Vec<Value>, native: Vec<Value>) -> Vec<Value> {
    ...
    if journal.iter().any(|event| {
        event.get("kind").and_then(Value::as_str) == Some("user")
...
mod tests {
    ...
    fn context_history_keeps_receipts_and_tool_identity_even_if_native_is_richer() {
        ...
        for prompt in [
            "hello",
            "hello\n\n<atelier-file-scope>policy</atelier-file-scope>",
            "mismatch",
        ] {
            let native = vec![
                serde_json::json!({"kind":"user","text":prompt}),
                serde_json::json!({"kind":"text","text":"recovered answer"}),
            ];
