...
mod tests {
    ...
    async fn paused_exchange_survives_restart_and_resumes_with_report() {
        ...
        s.threads()
            .lock()
            .await
            .upsert(json!({"id":"child", "status":"running"}), true)
            .unwrap();
        let request =
            json!({"targetThreadId":"child", "requestId":"pause-smoke", "text":"Isolated test"});
        let queued = crate::agent_mailbox::action_send_message(&s, "parent", &request)
            .await
            .unwrap();
        ...
        s.threads()
            .lock()
            .await
            .upsert(json!({"id":"child", "status":"idle"}), true)
            .unwrap();
        ...
        assert_eq!(
            s.mailbox()
                .lock()
                .await
                .get(queued["messageId"].as_str().unwrap())
                .unwrap()
                .error_code
                .as_deref(),
            Some(mcp_err::LINK_PAUSED)
        );
        let duplicate = crate::agent_mailbox::action_send_message(&s, "parent", &request)
            .await
            .unwrap();
            ...
            while let Some(delivery) = deliveries.recv().await {
                ...
                assert!(
                    !replies
                        .iter()
                        .any(
                            |reply| serde_json::from_str::<Value>(reply).unwrap()["type"]
                                == "error"
                        ),
                    "{replies:?}"
                );
        ...
        .await
        .expect("queued message delivered after resume");
        let report = crate::agent_mailbox::action_report_to_parent(
            &restarted,
            "child",
            &json!({"requestId":"report-smoke", "report":{"summary":"Child result"}}),
        )
        .await
        .unwrap();
        ...
        assert!(messages
            .iter()
            .any(|message| message.kind == "report" && message.text == "Child result"));
        tokio::time::timeout(std::time::Duration::from_secs(5), async {
            loop {
                if restarted
                    .mailbox()
                    .lock()
                    .await
                    .get(report["messageId"].as_str().unwrap())
                    .unwrap()
                    .status
                    == "delivered"
                {
        ...
        .await
        .expect("report delivered to parent");
        handle_unlink_thread(&restarted, &json!({"threadId":"child"})).await;
        assert!(
            crate::agent_mcp::authorize_target(&restarted, "parent", "child")
                .await
                .is_err()
        );
        worker.abort();
