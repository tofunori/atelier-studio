...
(function(){
  ...
  var CSS =
    ...
    +'border-radius:10px;padding:4px 6px;font:var(--fs-body, 13px) var(--ui-font,-apple-system,system-ui,sans-serif);color:var(--txt,#dbdfe5)}'
    +'.akBar.on{display:flex}'
    +'.akBar button{padding:0;width:26px;height:24px;font-size:var(--fs-body-s, 12px);background:transparent;color:var(--muted,#868d9a);display:inline-flex;align-items:center;justify-content:center;'
    ...
    +'border:1px solid var(--border,#3a4150);font-size:var(--fs-body, 13px)}'
    +'.akPill .x:hover{border-color:#5b6575;color:#fff}'
    +'.akPill .go{width:32px;height:32px;border-radius:50%;background:var(--accent,#c96442);color:#fff;font-size:var(--fs-title, 15px)}'
    ...
    +'box-shadow:0 14px 48px rgba(0,0,0,.55);font:calc(var(--ui-base-size, 15px) * 12.5 / 15) var(--ui-font,-apple-system,system-ui,sans-serif);color:var(--txt,#dbdfe5)}'
    ...
    +'.akTgMenu .it .app{flex:none;font-size:var(--fs-caption, 10px);font-weight:600;letter-spacing:.05em;text-transform:uppercase;'
    ...
    +'.akTgMenu .hd{font-size:var(--fs-label, 11px);color:var(--muted,#9aa3b2);text-transform:uppercase;letter-spacing:.05em;padding:5px 9px 3px}';
