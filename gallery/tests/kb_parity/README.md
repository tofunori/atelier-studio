...
Contrat argv/stdin -> JSON de la chaîne KB, figé par exécution RÉELLE du CLI
Node historique (`sidecar/kb_cli.mjs`, retiré du dépôt le 2026-09-14) et
respecté à l'identique par le binaire Rust `atelier-kb-rs`, désormais la
référence. Rejoué par `../kb_parity.test.mjs` contre ce binaire (résolu
automatiquement, voir l'en-tête du harnais ; `KB_PARITY_BIN=<chemin>` pour
en imposer un autre) — fait partie de `npm run verify` (`test:kb:parity`) :
