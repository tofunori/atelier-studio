...
await page.locator('#annotPillN').click();assert.equal(await page.locator('#annotItems button').count(),2);await page.locator('#annotItems button').first().click();assert.equal(await page.locator('#annotNote textarea').inputValue(),'Premier commentaire');await page.locator('.annot-close').click();
...
await page.locator('#lbFit').click();assert.equal(await page.locator('#lbWrap').evaluate(el=>el.style.transform),'');
