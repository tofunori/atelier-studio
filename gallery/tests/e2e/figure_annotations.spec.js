...
test('figure annotation tools, explicit save, cancellation, session state and single send',async({page},testInfo)=>{
...
await page.goto('http://localhost/');await page.evaluate(async()=>{lbList=[{rel:'one.png',name:'one.png',ext:'png'},{rel:'two.png',name:'two.png',ext:'png'}];lbIdx=0;lb().classList.add('show');let img=document.getElementById('lbImg');img.src='data:image/svg+xml,'+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="800" height="600"><rect width="800" height="600" fill="white"/></svg>');await img.decode();await annotToggle();});
...
await page.locator('#annotFit').click();assert.equal(await page.evaluate(()=>lbZoomLevel),1);
...
await drag(120,120,250,210);await page.locator('#annotNote textarea').fill('Premier commentaire');await page.locator('.annot-save').click();assert.equal(await page.evaluate(()=>annotStrokes.length),1);assert.equal(await page.locator('#annotNote').isVisible(),false);
await page.locator('[data-tool="arrow"]').click();await drag(420,320,320,320);await page.locator('#annotNote textarea').fill('Flèche gauche');await page.locator('.annot-save').click();assert.equal(await page.evaluate(()=>annotStrokes[1].tool),'arrow');
await page.locator('#annotPillN').click();assert.equal(await page.locator('#annotItems button').count(),2);await page.locator('#annotItems button').first().click();assert.equal(await page.locator('#annotNote textarea').inputValue(),'Premier commentaire');await page.locator('.annot-save').click();
...
await page.evaluate(()=>annotAskNote(annotStrokes[0],300,250));await page.locator('#annotNote textarea').fill('Modification annulée');await page.locator('.annot-close').click();assert.equal(await page.evaluate(()=>annotStrokes[0].note),'Premier commentaire');
await page.locator('[data-tool="rect"]').click();await drag(450,430,540,510);await page.locator('#annotNote textarea').fill('Brouillon annulé');await page.locator('.annot-close').click();assert.equal(await page.evaluate(()=>annotStrokes.length),2);
...
await page.locator('.annot-send').click();await page.waitForFunction(()=>annotBusy);assert.equal(await page.locator('#annotPillSend').isDisabled(),true);assert.equal(await page.evaluate(()=>annotGuard()),false);assert.equal(saves.length,1);assert.equal(saves[0].notes.length,1);releaseSave();await page.waitForFunction(()=>!annotBusy);assert.equal(await page.evaluate(()=>annotStrokes.length),1);
