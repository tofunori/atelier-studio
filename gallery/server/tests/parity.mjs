...
async function main() {
  ...
  assert.match(themeBridge, /dataset\.shadcnContract = "gallery-v1"/, "all gallery viewers receive semantic aliases");
  assert.match(reactToolbar, /components\/shadcn\/dropdown-menu/, "gallery toolbar imports project shadcn primitives");
  assert.match(reactToolbar, /components\/shadcn\/input-group/, "gallery search uses the project shadcn input group");
  assert.match(reactToolbar, /components\/ui\/Tooltip/, "gallery icon actions use the shared shadcn tooltip adapter");
  assert.doesNotMatch(reactToolbar, /type="search"/, "gallery search does not duplicate native search styling");
  assert.doesNotMatch(reactToolbar, /tw:z-(?:50|\[201\])/, "gallery consumers do not own overlay stacking");
