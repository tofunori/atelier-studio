...
function Demo() {
    ...
    const urls = { gallery: origin + '/figures_index.html', editor: origin + '/.fig_thumbs/code_editor.html?path=' + encodeURIComponent(root + '/analysis.py'), reading: origin + '/.fig_thumbs/pdf_viewer.html?file=observation-windows.pdf' };
    const surface = scene in urls ? scene as keyof typeof urls : 'reading';
    const tabs = [{ id: 'reading', title: 'observation-windows.pdf', kind: 'document' as const }, { id: 'editor', title: 'analysis.py', kind: 'document' as const }, { id: 'gallery', title: 'Gallery', kind: 'surface' as const, surface: 'atelier' as const }];
