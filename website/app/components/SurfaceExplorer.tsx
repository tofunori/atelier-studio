...
const surfaces = [
    ...
    { id: 'reading', name: 'PDF reader', description: 'Read published research beside the conversation. Shown here: the original brms paper by Paul-Christian Bürkner (2017).' },
...
export default function SurfaceExplorer() {
  ...
  <figure><a href={assetPath(`/media/${active.id}.png`)} target="_blank" rel="noreferrer" aria-label={`Open full-size ${active.name} screenshot`}><img src={assetPath(`/media/${active.id}.png`)} width="1600" height="1000" alt={`Atelier ${active.name}, showing the fictional Observatory project`} loading="lazy"/></a><figcaption aria-live="polite"><span>{active.description}</span><span>{active.id === 'reading' ? <><a href="https://doi.org/10.18637/jss.v080.i01">Bürkner (2017), Journal of Statistical Software</a> · <a href="https://creativecommons.org/licenses/by/3.0/">CC BY 3.0</a> · Original PDF</> : 'Fictional demo project · Open image to enlarge'}</span></figcaption></figure>
 </div>;
}
